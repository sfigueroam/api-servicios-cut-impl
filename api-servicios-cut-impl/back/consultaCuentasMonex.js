...
module.exports.consultaCuentasMonex = async(event, context, callback) => {
    ...
    let resultadoServicio = await servTierra.consultaCuentasMonex(tokenCache, urlParameters);
    
    //Si Falla el token volvemos a invocar token y servicio Tierra
    if (resultadoServicio.codeStatus == 401)    {
         
         tokenCache = await genToken.obtenerToken(clienteId, scope, clientSecret, grantType);
         resultadoServicio = await servTierra.consultaCuentasMonex(tokenCache, urlParameters);
    }
   
    console.log("resultadoServicio: ", resultadoServicio);
    send(resultadoServicio,callback);
    console.log("[FIN] consultaCuentasMonex");
};
