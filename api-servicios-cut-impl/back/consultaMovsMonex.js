...
module.exports.consultaMovsMonex = async(event, context, callback) => {
    
    console.log("[INICIO] consultaMovsMonex");
   ...
   let resultadoServicio = await servTierra.consultaMovsMonex(tokenCache, urlParameters);
    
    //Si Falla el token volvemos a invocar token y servicio Tierra
    if (resultadoServicio.codeStatus == 401)    {
         
         tokenCache = await genToken.obtenerToken(clienteId, scope, clientSecret, grantType);
         resultadoServicio = await servTierra.consultaMovsMonex(tokenCache, urlParameters);
    }
   
    console.log("resultadoServicio: ", resultadoServicio);
    send(resultadoServicio,callback);
    console.log("[FIN] consultaMovsMonex");
